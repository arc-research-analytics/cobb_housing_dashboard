...
def load_tab_data():
    # load the data
    df = pd.read_csv('Cobb_18-23_dash.csv',
                     thousands=',',
...
# map variables
map_lat_2D = 33.93866170628395
map_lat_3D = 34.90634
map_long = -84.57636700383478
map_height = 330
...
def mapper_2D():
    ...
    # read in geospatial
    gdf = gpd.read_file('Geography/Cobb_geos.gpkg')
...
def chart_builder():
    ...
    # because there are only 4 sales in the latest month-year category (June 2023), it's dragging down that
    # monthly average. So, I'm eliminating those sales. The dash will only go thru May 2023 now.
    df_new = df[df['year-month'] != '2023-6']

    df_grouped = df_new.groupby('year-month').agg({
        'price_sf': 'median',
        'unique_ID': 'count',
